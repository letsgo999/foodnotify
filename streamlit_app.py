...
supplement_id_check = st.number_input('확인할 건강기능식품 ID', min_value=1)

if st.button('소진 날짜 계산'):
    c.execute('SELECT name, start_date, current_stock, daily_intake FROM supplements WHERE id = ?', (supplement_id_check,))
    result = c.fetchone()
    if result:
        ...
        st.error('해당 ID의 건강기능식품이 존재하지 않습니다.')
